...
while count <= 50:
    ...
    with webdriver.Chrome(service=Service(ChromeDriverManager().install()),
        ...
        driver.get(
            f"https://www.vseinstrumenti.ru/category/akkumulyatornye-dreli-shurupoverty-15/page{count}/")  # Открываем страницу
        ...
        for i in block:
            ...
            with webdriver.Chrome(service=Service(ChromeDriverManager().install()),
                ...
                storage = {'name': head, 'price': cena, 'code': art, 'params': '; '.join(value), 'photo': '$'.join(img),
                           'URL': w}
                with open('shurupoverty.csv', 'a+', encoding='utf-16') as file:
